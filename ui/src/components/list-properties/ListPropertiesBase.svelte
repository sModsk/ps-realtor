<script lang="ts">
	...
	import { SHELLS, TEMP_HIDE, SM_DATA, SM_SHELLS } from '@store/stores'

	const propertyTypes = ['mlo', 'shell', 'smShell']
	let currentType = 'mlo'
	...
	$: {
		valid =
			description.length > 0 && price > 0 && (currentType === 'mlo' || currentType === 'shell' && shell.length > 0 || currentType === 'smShell' && shell.length > 0) && elements.every(element => element.door_data !== false)
			...
			<div class="right-column">
				<div id="property-type" class="form-row-wrapper">
					...
					<FormWrapperDropdown
						...
						currentType = event.detail; // Set currentType based on selection

						// Check if currentType is "smShell" and update shell and shellId accordingly
						if (currentType === "smShell") {
							const shells = $SM_SHELLS;
							shell = shells.length > 0 ? shells[0].name : "";
							shellId = shells.length > 0 ? shells[0].id : "";
						}
						}}
					/>
					</div>
				...
				<div id="door-creation" class="form-row-wrapper">
					...
					{#each elements as element, index}
                        {#if currentType === 'mlo' || (currentType === 'shell' && index === 0) || (currentType === 'smShell' && index === 0)}
				...
				{#if currentType === 'smShell'}
					...
					{#if $SM_DATA.allowPlayerModifications}
						<div id="garden-creation" class="form-row-wrapper">
							<p> Pre-created shell or base template </p> 
							<div class="action-row">
								<button 
									class="regular-button" 
									on:click={() => {
										publicShell = !publicShell;
										if (publicShell) {
										shell = $SM_SHELLS.length > 0 ? $SM_SHELLS[0].name : "";
										shellId = $SM_SHELLS.length > 0 ? $SM_SHELLS[0].id : "";
										}
									}}>
									{publicShell ? "Pre-created" : "Template"}
								</button>
							</div>
						</div>
					{/if}

					{#if $SM_DATA.allowPlayerModifications && !publicShell}
						<div id="shell-size" class="form-row-wrapper">
							<p class="label">Shell Size</p>
							<div class="action-row">
							<FormWrapperDropdown
								dropdownValues={$SM_DATA.shellSizes.map(shell => shell.name)}
								label=""
								id="new-listing-dd-shell-size"
								selectedValue={$SM_DATA.shellSizes.length > 0 ? $SM_DATA.shellSizes[0].name : ""}
								insideLabel="Size: "
								on:selected-dropdown={(event) => {
									customShell = $SM_DATA.shellSizes.findIndex(shell => shell.name === event.detail);
								}}
							/>
							</div>
						</div>
					...
					{#if publicShell}
						<div id="shell-type" class="form-row-wrapper">
							...
							<div class="action-row">
								<FormWrapperDropdown
								dropdownValues={$SM_SHELLS.map(shell => shell.name)}
								label=""
								id="new-listing-dd-shell-type"
								selectedValue={$SM_SHELLS.length > 0 ? $SM_SHELLS[0].name : ""}
								insideLabel="Type: "
								on:selected-dropdown={(event) => {
									const selectedShell = $SM_SHELLS.find(shell => shell.name === event.detail);
									if (selectedShell) {
